  ...
  Widget build(BuildContext context) {
    return ScopedModelDescendant<AppStateModel>(
        builder: (context, child, model) {
          return Builder(
              builder: (context) {
                print(widget.appBarStyle.appBarType);
...
class _MyDrawerState extends State<MyDrawer> {
  ...
  _buildList(AppStateModel model) {
    ...
    Color foreGroundColor = isDark ? Colors.white : model.blocks.settings.menuTheme.light.canvasColor.isDark ? Colors.white : Colors.black;
...
class _ZooDrawerAppState extends State<ZooDrawerApp> with TickerProviderStateMixin {
    ...
    if(bottomItemsLength < 2) {
      ...
      widget.appStateModel.blocks.settings.bottomNavigationBar.items.forEach((element) {
        if(element.link == 'home') {
          ...
          _children.add(BlockPage(child: MenuItem(linkType: 'template', linkId: id, title: element.title)));
          ...
          var child = MenuItem(linkId: element.linkId!, linkType: 'post');
          _children.add(WPPostPage(child: child));
        } else if(element.link == 'page') {
          var child = MenuItem(linkId: element.linkId!, linkType: 'page');
          _children.add(WPPostPage(child: child));
      ...
      return ScopedModelDescendant<AppStateModel>(
          builder: (context, child, model) {
            if (model.loading) {
              ...
              //TODO Place Picker Removed
              /*return Scaffold(body: PlacePickerHome());*/
  ...
  void _onMessage(RemoteMessage message) {
    if (message.data.isNotEmpty) {
      if (message.data.containsKey('category')) {
        ...
        var child = MenuItem(linkId: message.data['page'].toString(), linkType: 'page');
        Navigator.push(context,
            MaterialPageRoute(builder: (context) => WPPostPage(child: child)));
      } else if (message.data.containsKey('post')) {
        var child = MenuItem(linkId: message.data['post'].toString(), linkType: 'post');
        Navigator.push(context,
            MaterialPageRoute(builder: (context) => WPPostPage(child: child)));
  ...
  Future<void> navigateTo(Uri deepLink) async {
    ...
    if (deepLink.queryParameters['category'] != null) {
      ...
      var child = MenuItem(linkId: deepLink.queryParameters['post'].toString(), linkType: 'page');
      Navigator.push(context,
          MaterialPageRoute(builder: (context) => WPPostPage(child: child)));
    } else if (deepLink.queryParameters.containsKey('post')) {
      var child = MenuItem(linkId: deepLink.queryParameters['post'].toString(), linkType: 'post');
      Navigator.push(context,
          MaterialPageRoute(builder: (context) => WPPostPage(child: child)));
