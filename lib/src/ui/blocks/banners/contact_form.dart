    ...
    list.add(StreamBuilder<Contact7FormResult>(
      ...
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          ...
          child: Text(snapshot.data!.message, textAlign: TextAlign.center, style: Theme.of(context).textTheme.bodyMedium!.copyWith(
            color: snapshot.data!.status == 'mail_sent' ? Colors.green : Theme.of(context).colorScheme.error
          ),),
        );
        } else {
          return Container();
