  ...
  List<Widget> _buildAccountGroup(MenuGroup accountGroup, AppStateModel model) {
    ...
    if (accountGroup.showTitle) {
      groupWidgets.add(
        SliverToBoxAdapter(
          child: ListTile(
            subtitle: Text(accountGroup.title, style: Theme.of(context).textTheme.headlineSmall),
          ),
        ),
      );
    } else {
            ...
            return Column(
              children: [
                ListTile(
                  ...
                  leading: menuItem.leading.isNotEmpty ? ColoredIcon(item: menuItem) : null,
                  trailing: menuItem.trailing.isNotEmpty
                      ? DunesIcon(iconString: menuItem.trailing, color: Theme.of(context).brightness == Brightness.light ? Color(0xff8a8a8d) : null)
                      : null,
                  title: Text(
                    parseHtmlString(menuItem.title),
                  ...
                  subtitle: menuItem.description.isNotEmpty
                    ...
                    style: Theme.of(context).textTheme.bodyMedium!.copyWith(
                      color: Theme.of(context).brightness == Brightness.light ? Color(0xff8a8a8d) : Colors.grey,
                      letterSpacing: 0,
                    ),
                  )
                      : null,
