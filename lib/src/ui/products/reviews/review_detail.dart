...
class _ReviewsDetailState extends State<ReviewsDetail> {
        ...
        builder: (context, AsyncSnapshot<List<ReviewModel>> snapshot) {
          if (snapshot.hasData) {
            return CustomScrollView(
              slivers: [
                SliverToBoxAdapter(
                  child: buildReviewSummary(snapshot.data!, context),
                ),
                buildReviewsList(snapshot.data!, context),
              ],
            );
          } else {
            return Container();
  ...
  SliverList buildReviewsList(
      List<ReviewModel> reviews, BuildContext context) {
    return SliverList(
      delegate: SliverChildBuilderDelegate(
          ...
          return Column(
            ...
            children: [
              ...
              Divider(height: 0.0, thickness: 0),
            ],
          );
        },
  ...
  Widget buildListTile(BuildContext context, ReviewModel comment) {
    return Padding(
  ...
  Column buildReviewSummary(List<ReviewModel> snapshot, BuildContext context) {
    var fiveStars = snapshot.where((element) => double.parse(element.rating) > 4 && double.parse(element.rating) <= 5).length;
    var fourStars = snapshot.where((element) => double.parse(element.rating) > 3 && double.parse(element.rating) <= 4).length;
    var threeStars = snapshot.where((element) => double.parse(element.rating) > 2 && double.parse(element.rating) <= 3).length;
    var twoStars = snapshot.where((element) => double.parse(element.rating) > 1 && double.parse(element.rating) <= 2).length;
    var singleStars = snapshot.where((element) => double.parse(element.rating) > 0 && double.parse(element.rating) <= 1).length;
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${snapshot.length} ${AppStateModel().blocks.localeText.reviews}',
                      style: Theme.of(context).textTheme.titleMedium!.copyWith(color: Colors.grey),
                    ),
                  ],
                ),
  ...
  Stack buildBarStack(int value, int count) {
    final width = MediaQuery.of(context).size.width - 270;
    return Stack(
      children: [
        ClipRRect(
