...
class AppStateModel extends Model {
  ...
  static AppStateModel get instance => _appStateModel;
  ...
  Future<bool> firebasePhoneLogin(data, BuildContext context) async {
    loginLoading = true;
    notifyListeners();
    final response = await apiProvider.postWithCookies(
    ...
    if (response.statusCode == 200) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(parseHtmlString(parseHtmlString(error.data[0].message))),
  ...
  Future<bool> phoneLogin(Map<String, dynamic> data, BuildContext context) async {
    loginLoading = true;
    notifyListeners();

    // Make the OTP verification request
    final response = await apiProvider.postWithCookies('/wp-admin/admin-ajax.php?action=taqnix_verify_otp', data);
    ...
    if (response.statusCode == 200) {
      ...
      if (user.id > 0) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(parseHtmlString("Failed to retrieve user details.")),
            backgroundColor: Colors.red,
          ),
        );
        return false;
      ...
      // Handle specific OTP verification failure (400 error)
      WpErrors error = WpErrors.fromJson(json.decode(response.body));
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(parseHtmlString(error.data[0].message)),
          backgroundColor: Colors.red,
        ),
      );
      return false;
    } else {
      // General error handling for any other status code
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(parseHtmlString("An unexpected error occurred.")),
          backgroundColor: Colors.red,
        ),
      );
      return false;
  ...
  getDeliverySlot(dateParam) async {
    ...
    formData['nonce'] = blocks.nonce.jckwds;//'5c895bbe90';
  ...
  deleteAccount(BuildContext context) async {
    ...
    if (response.statusCode == 200) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(error.data[0].message),
        ),
      );
